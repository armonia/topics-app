...
function deps(over: Partial<ProbePortDeps> = {}): ProbePortDeps {
  return {
...
describe("chi risponde sulla nostra porta", () => {
  it("riconosce la nostra risposta e tace", async () => {
    const e = await probePort(3333, deps());
    expect(e).toEqual({ stato: "nostro" });
    expect(verdictMessage(3333, e)).toBeNull();
  });
  ...
  it("IL CASO VERO: risponde 200 ma con l'HTML di un altro progetto", async () => {
    ...
    const msg = verdictMessage(3333, e)!;
  ...
  it("nessuno che risponde NON è un allarme", async () => {
    ...
    const e = await probePort(3333, deps({ chiedi: async () => null }));
  ...
  it("un estraneo NON identificabile viene comunque denunciato", async () => {
    ...
    expect(verdictMessage(3333, e)).toContain("non sono riuscito a identificare");
  });
  ...
  it("una sonda che ESPLODE non diventa un allarme", async () => {
    ...
    expect(verdictMessage(3333, e)).toContain("ECONNRESET");
  });
  ...
  it("interroga la rotta più economica su 127.0.0.1, provando HTTPS per primo", async () => {
    ...
    const visti: string[] = [];
    await probePort(3333, deps({ chiedi: async (u) => { visti.push(u); return { ok: true, corpo: NOSTRA }; } }));
    expect(visti).toEqual([`https://127.0.0.1:3333${PROBE_ROUTE}`]);
  });
  ...
  it("la forma conta, non il codice: JSON di un altro server non passa", () => {
    expect(isOurResponse(NOSTRA)).toBe(true);
    expect(isOurResponse('{"status":"ok"}')).toBe(false);
    expect(isOurResponse('{"openSessions":"tre","workingSessions":1}')).toBe(false); // tipo sbagliato
    // UN campo solo non basta: potrebbe capitare per caso altrove.
    expect(isOurResponse('{"openSessions":19}')).toBe(false);
