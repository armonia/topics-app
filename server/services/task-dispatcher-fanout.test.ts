...
function harness(overrides: Partial<DispatcherDeps> = {}) {
  ...
  const deps: DispatcherDeps = {
    ...
    getLastAgentText: (sk) => `riassunto di ${sk}`,
...
describe("task-dispatcher fan-out", () => {
  ...
  it("il tetto stringe il fan-out invece di rifiutarlo, e lo dice", async () => {
    ...
    // Il testo è quello accorciato in 76ea5396 ("i messaggi di sistema dicono
    // la stessa cosa in metà delle parole"): il commit cambiò la stringa e non
    // questa attesa, lasciando il rosso su main.
