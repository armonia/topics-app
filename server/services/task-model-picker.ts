...
export const CLASSIFIER_PROMPT = (title: string, description: string) =>
  [
    ...
    "- heavy: il lavoro passa per forza da una macinata lunga che occupa QUESTA macchina — compilare/buildare il progetto, la suite di test intera, installare dipendenze, indicizzare, benchmark, encoding video. Nel dubbio light.",
    "- Conta DOVE gira la macinata, non che esista: se il task la manda su un altro host (un PC remoto via ssh, la CI, un server di produzione) e qui resta solo l'attesa, è light. Un heavy ferma tutta la coda finché non finisce, e fermarla per un lavoro che consuma un'altra macchina è un blocco senza motivo.",
    "",
