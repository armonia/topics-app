  ...
  function buildKickoff(task: Task): string {
    ...
    parts.push(
      [
        ...
        "  · TAB — una pagina viva da far vedere o navigare al reviewer (dev server, report HTML, dashboard, pagina) la apri TU con open_browser_pane({url}): dentro un task quella diventa una tab DEL TASK, resta nel task dopo la fine del tuo turno ed è lì che il reviewer la trova. Una tab per superficie che serve davvero (l'app, il report), non di più.",
