...
export interface DispatcherDeps {
  ...
  usageTickMs?: number;
  /**
   * THE REMOTE LANE: absent means no card can run anywhere but here.
...
export interface TaskDispatcher {
  ...
  busyIds(): string[];
  /**
   * The sessions those runs are bound to (setup slots, still without one, are
   * left out). A card turn streams through /api/chat, so the restart gate
   * needs these keys to tell a card's own stream from a person's chat.
   */
...
import { machineBudgetMessage, memoryClause } from "./dispatch-budget-message";
...
export function createTaskDispatcher(deps: DispatcherDeps): TaskDispatcher {
  ...
  /**
   * THE REMOTE LANE, whole, in `task-dispatcher-remote-node.ts`. What stays
   * here are its five seams: launch, liveness sweep, burial, poll, and the
   * requeue after a restart.
   */
  ...
  function buryDeadRun(taskId: string, runId: number): void {
    ...
    if (slot.node) { remote.buryRun(taskId, slot.node); return; }
    const lt = liveTurns.get(taskId);
    endLiveTurn(taskId);
  ...
  function sweepDeadTurns(): Set<string> {
    ...
    for (const [taskId, slot] of inFlight) {
      // A REMOTE slot is judged by its own lane (consecutive failed polls),
      // never by the local probe nor by the clock: see that module.
      const verdict = remote.sweepVerdict(slot);
      if (verdict !== "local") {
        if (verdict === "bury") doomed.push({ taskId, runId: slot.runId });
        continue;
      }
