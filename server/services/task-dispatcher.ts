...
export interface DispatcherDeps {
  ...
  usageTickMs?: number;
  /**
   * THE REMOTE LANE: how this board talks to a paired node, and how a branch
   * born there is planted here (KANBAN-76, KANBAN-77).
   *
   * Optional like every other host capability: absent means no card can run
   * anywhere but here, which is the historical behaviour and what every test
   * harness that does not care about nodes keeps getting.
   *
...
export function createTaskDispatcher(deps: DispatcherDeps): TaskDispatcher {
  ...
  interface RunSlot {
    ...
    deadSweeps: number;
    /** Set only on a REMOTE slot: the run this card is riding on a node. */
    node?: NodeSlot;
  }
  ...
  function nodeName(machineId: string): string {
    try { return deps.node?.nameOf(machineId) || machineId; } catch { return machineId; }
  }
  ...
  function deferForSilentNode(task: Task, why: string): void {
    try {
      const after = deferWait(task.id, why, NODE_RETRY_MINUTES);
      ...
      if (after.status === "todo") {
        emit(deps.svc.setDispatchState({
          taskId: task.id,
  ...
  async function launchOnNode(
    ...
    try {
      ...
      bindRunSession(task.id, runNo, nodeSessionKey(machineId, runId));
      const slot = inFlight.get(task.id);
      if (slot) slot.node = { machineId, baseUrl, token, runId, fails: 0, seen: new Set(), primed: false };
      emit(deps.svc.setDispatchState({ taskId: task.id, state: CHIP_WORKING }));
      try {
        deps.svc.addComment({
  ...
  async function launch(
    ...
    // names a node: no worktree, no topic, no turn (KANBAN-76). It sits before
    // `beginRun` because the remote slot keeps its handle after this function
    // returns, and the `finally` below would free it under the poll.
    const claimedTask = deps.svc.get(taskId)?.task;
    const localMachine = (() => { try { return deps.node?.localMachineId?.() ?? null; } catch { return null; } })();
    if (claimedTask?.machineId && claimedTask.machineId !== localMachine) {
      if (!deps.node) {
        parkNodeCard(
          claimedTask,
      ...
      await launchOnNode(claimedTask, settings);
      return;
    }
   ...
   */
  function buryNodeRun(taskId: string, node: NodeSlot): void {
  ...
  function buryDeadRun(taskId: string, runId: number): void {
    ...
    // A REMOTE run has no process, no session and no live turn to account for:
    // the accounting below would book a transcript that does not exist and end
    // a turn that never started here.
    if (slot.node) { buryNodeRun(taskId, slot.node); return; }
    const lt = liveTurns.get(taskId);
    endLiveTurn(taskId);
  ...
  function sweepDeadTurns(): Set<string> {
    ...
    for (const [taskId, slot] of inFlight) {
      ...
      // `isTurnAlive` knows nothing about a session on another machine and
      // would answer "dead" for every one of them, burying a healthy run in two
      // sweeps. And never by elapsed time: a closed laptop produces no polls at
      // all, so silence alone buries nothing (KANBAN-77).
      if (slot.node) {
        if (slot.node.fails >= NODE_DEAD_POLLS) doomed.push({ taskId, runId: slot.runId });
        continue;
      }
      if (!probe) continue;                                 // no local probe: the historical behaviour
  ...
  function mirrorNodeReport(taskId: string, node: NodeSlot, report: NodeRunReport): void {
    const got = deps.svc.get(taskId);
    if (!got) return;
    if (report.dispatchState && report.dispatchState !== got.task.dispatchState) {
      try { emit(deps.svc.setDispatchState({ taskId, state: report.dispatchState })); }
  ...
  async function landNodeDelivery(taskId: string, node: NodeSlot, report: NodeRunReport): Promise<void> {
    ...
    try {
      const delivered = deps.svc.deliverToReviewBySystem({ taskId, reason, nextMove });
      emit(delivered);
   ...
   */
  async function pollNodeRuns(): Promise<void> {
    const client = deps.node;
    if (!client) return;
    for (const [taskId, slot] of [...inFlight]) {
  ...
  async function reconcile(opts?: { reason?: "boot" | "poll" }): Promise<void> {
    ...
    // 0-bis) THE REMOTE RUNS, polled BEFORE the sweep: a failure counted now is
    ...
    for (const t of running) {
      ...
      // A card that names a NODE is never resumed HERE. The recovery below ends
      // in `resume`, which opens a local topic and a local turn: for a remote
      ...
      if (t.machineId && deps.node) {
        try {
          ...
          releaseAndEmit({ taskId: t.id, requeue: true, rollbackAttempt: true });
          inCoda++;
        } catch (err) { log(`nodo: card ${t.id} non rimessa in coda dopo il riavvio`, err); }
        continue;
      }
