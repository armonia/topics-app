...
const giroTagliatoDopoLaChiamata = sse([
  { type: "message_start", message: { usage: { input_tokens: 10 } } },
...
const giroTroncatoDalTetto = sse([
  { type: "message_start", message: { usage: { input_tokens: 10 } } },
...
function handler(reg: Registro): StreamHandler {
  return {
    onTextDelta: () => {},
    onToolStart: () => {},
    onToolResult: () => {},
...
describe("il giro che muore a meta' non e' una fine naturale", () => {
  beforeAll(() => {
    casa = mkdtempSync(join(tmpdir(), "native-cut-home-"));
  ...
  test("stop_reason assente e una chiamata a strumenti aperta: la fine NON e' end_turn", async () => {
    const { out, reg } = await turno(giroTagliato);
  ...
  test("stessa fine anche se la chiamata era gia' chiusa: quel che conta e' che il giro non si sia chiuso con tool_use", async () => {
    const { out } = await turno(giroTagliatoDopoLaChiamata);
    expect(out.turnEnd.end).toBe("error");
  });

  test("il tetto dei token resta max_tokens: il taglio ha una ragione, e si dice quella", async () => {
    const { out, reg } = await turno(giroTroncatoDalTetto);
    expect(out.turnEnd.end).toBe("max_tokens");
    expect(reg.done).toBe(1);
  });

  test("un turno sano finisce end_turn, e nessuno lo tocca", async () => {
    const { out, reg } = await turno(giroSano);
  ...
  test("un turno che non produce nemmeno una parola, ma si chiude, resta end_turn", async () => {
    const { out } = await turno(giroMuto);
    expect(out.turnEnd.end).toBe("end_turn");
  });
});
