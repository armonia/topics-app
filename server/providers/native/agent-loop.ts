 ...
 */
/** Exported for the test: this is the function that decides a round's
 *  verdict, and the 21/09 fault (a refusal with no `cause`, hence no
 *  banner) lived exactly in one of its branches. */
export function roundEnd(
  ...
  if (stopReason === "refusal") {
    // THE CAUSE TRAVELS WITH THE VERDICT, or the verdict stays unseen.
    //
    ...
    // lacks it). Without it, the refusal stayed a block at the bottom of
    // the bubble: on topic:a5c4a915, 21/09, the 19th of 19 under a pile of
    // tool calls, and the chat looked "stuck with no feedback".
