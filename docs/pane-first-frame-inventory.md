...
| Chat inside a project window, cache present | **was:** skeleton curtain until the WHOLE server history landed (544-1195 ms measured, cap 1200; 2.6 MB for 17 messages) | `POST /api/history` with `limit: HISTORY_FIRST_PAGE` (40), the rest with `before` only while the pane is hidden or from the "load older" divider | `messages-cache-<sessionKey>` now holds exactly the first page, so the local copy IS the page the server confirms | fixed: 175 ms on the desktop's real state (2026-09-06); `blocks[].toolCall.args/detail` over 512 chars travel as a head + byte count, loaded when the row opens |
