...
| Any lazy pane body, chunk already warm | **was:** `React.lazy` still committed the fallback on the first mount (136 ms measured with every chunk cached at 110 ms) | none: a microtask and the boot's own render work | **added:** `lib/lazyWarm` renders a warm module in the same pass, no boundary | fixed |
