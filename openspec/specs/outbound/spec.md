...
Ciò che Topics manda FUORI dalla macchina per conto di un agente: un messaggio
di posta, una chiamata alle API Google. Il confine è quello: dentro c'è il
lavoro, fuori c'è qualcuno che riceve, e ciò che esce non si annulla.

Da qui la forma dei requisiti: la configurazione non entra nel repo (pubblico),
il sì di una persona precede ogni invio e vale per QUEL messaggio, e ogni cosa
uscita lascia una traccia sulla card che l'ha prodotta.
