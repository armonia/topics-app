...
- [x] T1.1 Esenzione «prima card» sul pavimento della MEMORIA: con zero agenti
...
- [x] T1.2 Test: `admissionBlock` sotto il pavimento con `inFlight = 0` e
      `checkRuns = 0` ammette; con un agente vivo no; il disco pieno non ammette mai.
- [x] T1.3 I campioni del segnale di memoria sopravvivono al riavvio e si
      ricaricano applicando `MEM_SAMPLE_GAP_MS`. (28 finestre azzerate in 25,7 h,
      circa 56 min al giorno)
- [x] T1.4 Test: campioni di 3 s ereditati danno un minimo; campioni di 10 min no.
- [x] T1.5 Il riscaldamento della finestra non ruba la chiave di dedup al motivo
      vero, e il registro «gia' detto» decade al cambio della frase. (80 commenti
      su 80 con la frase sbagliata dopo il 15/09 17:49)
- [x] T1.6 Test: `holdKey` distingue riscaldamento e pavimento.
...
- [x] T2.1 `taskIdleDays` smette di contare `tasks.updated_at`, che il dispatcher
      riscrive ogni 60 s mentre trattiene la card. (7 card su 7, zero abbandoni
      in tutta la storia del log)
- [x] T2.2 Test: ultimo commento a 9 giorni e chip riscritto adesso danno 9 giorni.
- [x] T2.3 `WAIT_SERIES_MAX_MS` valutato anche fuori da `deferForWait`, con
      parcheggio `waited_out`. (2 card oltre il tetto da 45 e 31 h, zero parcheggi)
- [x] T2.4 Test: `wait_since` a 5 ore e nessun turno partito produce il parcheggio.
- [x] T2.5 Una card con `reopened_actor = 'human'` riparte con i commenti umani da
...
- [x] T2.6 Test: reconcile su una card bocciata da un umano porta il testo umano.
- [x] T2.7 Il tetto sulla durata vale solo su una serie ANCORA IN CORSO: con
...
- [x] T2.8 La guardia su `commentIds` copre tutti e tre i chiamanti di
...
- [x] T3.1 Run terminale senza verdetto: un rerun, e se non si puo' il referto
      nomina il commit nuovo. (15 delle ultime 100 sha sono gia' in quello stato)
- [x] T3.2 Il giro esce al primo rosso accertato, le altre righe CI diventano non
      misurate con quella ragione.
- [x] T3.3 La sonda del conflitto si consuma solo su una risposta conclusiva.
...
- [x] T3.4 `ownCommits === 0` e' non misurato, non due verdi.
- [x] T3.5 Il contratto fissa anche la forma dei nomi dei job e2e (un solo asse
      nella matrice).
- [x] T3.6 Durante l'attesa la card porta il link della PR e della run, e la riga
      di attesa non attribuisce la misura al cancello della board. (15 min ciechi
      per consegna)
- [x] T3.7 Il dettaglio della card dipinge `unknown` come non misurato, non rosso.
      (6 card storiche; con le righe CI `unknown` diventa l'esito piu' probabile
      dopo il verde)
- [x] T3.8 Test per ciascuno dei sette.

## Tornata 4 — quel che resta aperto dopo il land

- [x] T4.1 Un land che riallinea azzera `checks_commit` e dice che i verdetti CI
...
- [x] T4.2 La bozza di PR e il ramo remoto si chiudono dopo il land, il rifiuto
