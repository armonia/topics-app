...
- [x] Giro di correzioni sulla seconda porta: la quota 0,90 dichiarata PROVVISORIA (il `total` non e' mai stato nel log, 39 righe su 869 leggono `swapUsedGB` sopra 16,384 e il denominatore si muove a scatti da 1 GB) con `swapTotalGB` ora nella riga per poterla tarare, precisione/richiamo contati sulle 874 righe vive (44%->32% e 39%->70%, episodio piu' lungo da 4 a 19 minuti), via d'uscita temporale anche al ramo `wait: "swap"` (prima stava davanti al budget e un falso positivo cronico teneva i check senza scadenza), segno del debito da una sola `signed()` nelle quattro frasi che lo stampano, pid responsabile chiesto solo per i servizi XPC, `Topics Host.app` fra i marcatori nostri e famiglie sommate sul footprint
...
- [x] Il thrash STABILE passava per calmo (16/09, 11 righe sopra 10 pagine/s e 2 sostenute): seconda porta sul SOFFITTO dello swap (`swapPct >= 0,90` in OR col debito, totale parsato dalla stessa riga di `vm.swapusage`, `swapPct` nella riga `[memsig]`), e una coda trattenuta ora dice CHI tiene la memoria fuori da Topics (un solo `/bin/ps` al minuto sul battito, famiglie per bundle `.app` più esterno, XPC attribuiti al pid responsabile, prime tre voci in `[memsig]` e in coda alla frase del pavimento)
