...
- [ ] 0.0 **Chiave di pin unica per progetto** — `paneConfig.ts::pinKeyForPane` (:158-171)
...
- [ ] 0.1 **Migrazione al load** — in `useSidebarState`, `normalizePinKey` su ogni voce di
  `pinnedItems` + dedup (una volta, al caricamento). Test: uno stato con entrambe le forme
  carica un solo pin.
- [ ] 0.2 `client/src/lib/pinnedLayout.ts` — modulo puro. `type PinnedRow = { keys: string[];
...
- [ ] 0.3 `client/src/lib/pinnedLayout.test.ts` (`bun:test`) — chiavi non più fissate potate,
...
- [ ] 0.4 `client/src/lib/iconTint.ts` — colore dominante da un'icona same-origin
...
- [ ] 0.5 `client/src/lib/iconTint.test.ts` — assenza di icona ⇒ `null`, soglia di
  luminanza per il testo, memoizzazione.
- [ ] 0.6 `lib/dndTypes.ts` — `PINNED_TILE: 'application/x-pinned-tile'`.
- [ ] 0.7 `hooks/useSidebarState.ts` — `pinnedLayout: PinnedRow[]` in `SidebarState`
...
- [ ] 0.8 **CAS sul PUT** — `?base=<server_seq>` + retry sul 409 `stale_base`, sul modello di
...
- [ ] 0.9 Estendere `useSidebarState.test.ts`: il campo sopravvive al round-trip, un payload
  vecchio senza campo carica senza errori, il 409 riapplica invece di sovrascrivere.

## Phase 1 — La tessera
- [ ] 1.1 `components/Sidebar/PinnedTile.tsx` — `role="treeitem"`, `data-pinned="true"`,
...
- [ ] 1.2 Drag source nativo HTML5: `PINNED_TILE` **e** `PANEL_ID` sullo stesso
  `dataTransfer` (il drop nella griglia dei pane deve restare identico), ghost compatto
  come `TopicItem.handleDragStart`.
- [ ] 1.3 Menu contestuale della tessera: le stesse voci della riga (fissa/rimuovi,
  archivia, …) — nessuna funzione persa passando da riga a tessera.

## Phase 2 — La griglia, e via l'etichetta
- [ ] 2.1 `components/Sidebar/PinnedTiles.tsx` — righe da `pinnedLayout` riconciliato,
...
- [ ] 2.2 `TopicTree.tsx` — sostituire i 4 blocchi (`:999-1008`, `:1033-1044`,
  `:1052-1056`, `:1071-1075`) con `<PinnedTiles/>`. Spariscono le due intestazioni
  i18n e le due `renderSection('pinned', Pin, 'Fissati', …)`.
- [ ] 2.3 `lib/i18n.ts` — rimuovere `sidebar.pinnedSection` (it `:139`, en `:251`).
  **Tenere** `sidebar.pinned`: è il titolo del marcatore sulla riga, usato anche altrove.
- [ ] 2.4 Verificare che `pinnedBlock` resti l'unica fonte degli item (nessuna doppia
  resa: gli unpinned continuano a escludere i fissati).

## Phase 3 — Composizione delle righe in drag & drop
- [ ] 3.1 Drop target per riga: calcolo dell'indice d'inserimento dalla posizione X,
  tessera fantasma al posto d'inserimento.
- [ ] 3.2 Ridimensionamento in diretta: la riga sotto il cursore calcola `flex-basis` con
  `n+1` mentre il drag è sopra di lei; `transition: flex-basis 120ms`.
- [ ] 3.3 Zona di drop "riga nuova" sotto l'ultima riga (8px a riposo, riga intera in hover).
- [ ] 3.4 Al rilascio: `moveTile`/`appendRow` → `setPinnedLayout`. Annullamento pulito su
  `dragend` senza drop (nessuno stato fantasma appeso).
- [ ] 3.5 Verificare a mano che il drag di una tessera **dentro la griglia dei pane**
  continui ad aprire l'elemento (è il rischio numero uno di questa change).

## Phase 4 — Espansione sotto la riga
- [ ] 4.1 Stato aperto transitorio (`useState`, un `Set<string>`); click sulla tessera
  fa toggle. Nessuna persistenza.
- [ ] 4.2 Rendering: dopo ogni riga, la/e fascia/e delle sue tessere aperte. Contenuto =
  `item.children` con il `renderItem` esistente.
- [ ] 4.3 Più fasce aperte: contenitore in colonna, `flex: 1 1 0` per fascia, scroll
  proprio, tetto d'altezza della sezione.
- [ ] 4.4 Tastiera: `Enter`/`Space` sulla tessera fa toggle come il click; `Escape`
  chiude la fascia col focus dentro.

## Phase 5 — Verifica ed evidenza
- [ ] 5.1 `bun run typecheck:client` + `bun run build:client` verdi.
- [ ] 5.2 Test E2E esistenti dei Fissati verdi **senza modificarli**:
...
- [ ] 5.3 Nuovo `tests/e2e/sidebar-pinned-tiles.spec.ts`: tessere affiancate; nessuna
...
- [ ] 5.4 **Evidenza durevole = video**: `recordVideo` sul context per lo scenario di drag
