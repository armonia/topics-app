...
(function () {
   ...
   *
   * The mascot is the Claude Code mark in brand orange (#D97757), drawn as a
   * full-cell silhouette via the cell BACKGROUND (48;2 truecolor) — NOT via
   * block glyphs. WHY: xterm runs its DOM renderer here (deliberate, for native
   * mobile text selection), which paints each cell's glyph from the FONT.
   * Sub-cell glyphs — the quadrant/half blocks ▛▜▝▘▐▌ the real CLI logo uses —
   * don't meet at cell edges in a webfont, so they render with hairline seams
   * between the little blocks (and the Canvas/WebGL renderer that would draw
   * them seam-free isn't loaded). A cell background is a solid CSS rectangle
   * that fills the WHOLE cell with zero gaps regardless of font, so a silhouette
   * built from backgrounds tiles perfectly. We trade the sub-cell rounding of
   * the literal logo for a clean, seam-free mark — a rounded body with two feet,
   * the creature's recognizable shape. Eyes/holes are default-bg cells. Each row
   * is 8 cells wide so the trailing text aligns across all three lines. */
  var B = "\x1b[48;2;217;119;87m", R = "\x1b[49m"; // bg on / bg off
  var M1 = " " + B + "      " + R + " ";                       //  ██████
  var M2 = B + "        " + R;                                 // ████████
  var M3 = "  " + B + "  " + R + " " + B + "  " + R + " ";     //   ██ ██
  var CLAUDE_CC1 = [
