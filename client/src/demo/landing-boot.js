...
(function () {
  ...
  /* ---- 1a. App-level pane store: THREE project panes -------------------- */
  var appPanes = {};
  ...
  // Device-local grid overlay: SOLO each project into its own grid cell so the
  // three open projects lay out as — row 1: acme-web (full width); row 2:
  // acme-api | acme-mobile (side by side). The grid item key for a solo cell is
   ...
   *   acme-api (b-left):  a Claude session (working)
   *   acme-mobile (b-right): a Claude session (a COMPLETED turn)              */
  ...
  seedProject(P3, [
    ...
    rowHeights: [1], sidebarCollapsed: false, focusedGroupId: "pgC",
  });
