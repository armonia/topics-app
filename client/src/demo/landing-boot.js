...
(function () {
  ...
  /* ---- 1a. App-level pane store: TWO open project panes ----------------- *
   * acme-web + acme-api are OPEN (stacked split below). acme-mobile stays a
   * CLOSED project — it still shows in the sidebar (via workspaceProjects + its
   * running sessions), so the multi-project story survives without cramming a
   * third full window into the small hero. */
  var appPanes = {};
  ...
  // Device-local grid overlay: the two open projects STACK — acme-web on top,
  // acme-api below, each FULL WIDTH so they get real room at the small hero size
  // (3 windows here was too cramped). The grid item key for a solo cell is
  // `solo:<paneId>` (soloCells.ts soloCellKey) — it MUST match or PanelGrid's
  // additive-sync re-merges everything into row 0.
  set("topics-panel-grid-layout", JSON.stringify({
    gridRows: [
      { itemKeys: ["solo:" + PROJ_A], widths: [1] },
      { itemKeys: ["solo:" + PROJ_B], widths: [1] },
    ],
    gridRowHeights: [0.55, 0.45],
  ...
  seedProject(P2, [
    ...
    rowHeights: [1], sidebarCollapsed: false, focusedGroupId: "pgB",
  });
  // acme-mobile (P3) is intentionally NOT opened as a window — it shows in the
  // sidebar as a closed project with its running Claude Code sessions (cc4/cc5).
