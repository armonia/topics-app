...
export function AuraWave({ activityId, radius = DEFAULT_RADIUS }: AuraWaveProps) {
  ...
  useEffect(() => {
    ...
    // Trace the window-edge rounded rect onto the canvas — the outer boundary of
    // the fill (the inner boundary is the wave contour, via evenodd).
    ...
    const draw = (): void => {
      ...
      // FILL the region window-border → wave as nested rings anchored at the
      ...
      // band along the wave). evenodd: outer rounded rect minus the wave contour.
      ctx.fillStyle = grad;
      ...
      for (let Lr = 0; Lr < LAYERS; Lr++) {
        const f = (Lr + 1) / LAYERS;
        ctx.beginPath();
        roundRectTrace(cornerR); // outer boundary = window edge
