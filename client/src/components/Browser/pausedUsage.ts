...
import { maxPct, pctPlaceholders } from '../../lib/machineBusy';
...
export function pausedUsageText(
  ...
  return tr('browser.heavy.paused.body', pctPlaceholders(locale, { pct: pageSharePct(v, machine) }));
}
...
export function pageSharePct(v: { cpu: number; memMb?: number }, machine: { cores: number; memMb: number | null }): number {
  ...
  const pct = maxPct(share.cpuPct, share.memPct) ?? 0;
