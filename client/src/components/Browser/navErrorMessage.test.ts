...
/**
 * The catalogue lookup the pane does, minus React. The strip used to build its
 * sentences from Italian literals in this module; the words moved, the
 * assertions did not, so the test renders them the same way the screen does.
 */
...
describe('navErrorMessage', () => {
  ...
  test('la porta locale spiega anche PERCHÉ, che è quasi sempre un anteprima morta', () => {
    const t = navErrorMessage({ url: 'http://127.0.0.1:8791/report.html', description: '', code: -1004 }, say);
  ...
  test('stesso codice su un host vero: nessuna storia sulle anteprime', () => {
    const t = navErrorMessage({ url: 'https://example.com/x', description: 'Could not connect.', code: -1004 }, say);
  ...
  test('host inesistente, timeout e rete assente hanno frasi proprie', () => {
    expect(navErrorMessage({ url: 'https://nope.invalid/', description: '', code: -1003 }, say).message)
      .toContain('Indirizzo non trovato');
    expect(navErrorMessage({ url: 'https://slow.example/', description: '', code: -1001 }, say).message)
      .toContain('non ha risposto in tempo');
    expect(navErrorMessage({ url: 'https://x.example/', description: '', code: -1009 }, say).message)
  ...
  test('un codice sconosciuto tiene le parole di WebKit invece di nasconderle', () => {
    const t = navErrorMessage({ url: 'https://x.example/', description: 'Something odd happened.', code: -9999 }, say);
    expect(t).toEqual({ message: 'Something odd happened.' });
  });

  test('senza descrizione e senza URL leggibile resta almeno il codice', () => {
    expect(navErrorMessage({ url: '', description: '', code: -1004 }, say).message)
...
describe('deadLoopbackNotice', () => {
  test('dice chi non risponde e a che ora l\'abbiamo controllato', () => {
    const t = deadLoopbackNotice('http://localhost:3210/login', new Date(2026, 7, 5, 16, 3), say);
    expect(t.message).toContain('localhost:3210');
  ...
  test('l\'ora cambia fra due controlli: è il segnale che «Riprova» ha fatto qualcosa', () => {
    const a = deadLoopbackNotice('http://localhost:3210/', new Date(2026, 7, 5, 9, 5), say);
    const b = deadLoopbackNotice('http://localhost:3210/', new Date(2026, 7, 5, 9, 6), say);
...
describe('navigazione rifiutata dal guscio (-7001)', () => {
  ...
  test('non si confonde con un errore di rete vero', () => {
    const rete = navErrorMessage({ url: 'http://localhost:3210/', description: 'Could not connect to the server.', code: -1004 }, say);
