...
export function ChatPanel({
  ...
  return (
    <>
      <div role="region" aria-label={`${topic.name} panel`} className={`flex flex-col-reverse lg:flex-col flex-1 min-h-0 bg-surface overflow-hidden transition-all duration-100 ${isDragOver ? 'bg-primary/3' : ''}`} onClick={onFocus}>
        {/* Header */}
        <div className={`flex items-center gap-1.5 ${headerLeft ? 'pr-2' : 'px-2'} h-10 border-t lg:border-t-0 lg:border-b border-app-border select-none flex-shrink-0 bg-surface app-drag-region`}>
