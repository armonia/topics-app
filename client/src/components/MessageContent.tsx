...
export function ImageThumbnail({ file, onRemove }: { file: File; onRemove: () => void }) {
  ...
  return (
    <div className="relative inline-block">
      ...
      <button type="button" aria-label={tr('chat.attachments.removeImage')} onClick={onRemove} className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center hover:bg-red-600"><X className="w-3 h-3" aria-hidden="true" /></button>
    </div>
  );
}
