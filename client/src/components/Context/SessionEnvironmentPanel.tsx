...
export function SessionEnvironmentPanel({ topicId }: { topicId: string }) {
  ...
  if (error) {
    return (
      <div data-testid="session-env-error" className="flex items-start gap-2 px-4 py-2 text-mini text-red-500">
  ...
  if (!env) {
    return <div data-testid="session-env-loading" className="px-4 py-2 text-mini text-app-text-muted">{t('sessionEnv.loading')}</div>;
  }
  if (!env.inherits) {
    ...
    // and false statement.
    return (
      <div data-testid="session-env-no-inherit" className="px-4 py-2 text-mini text-app-text-muted">
  ...
  return (
    <div data-testid="session-env" className="px-4 pb-3 pt-1 space-y-3">
      <p className="break-words text-mini text-app-text-muted">{t('sessionEnv.blurb')}</p>
      ...
      <Section icon={<Plug size={11} />} title={t('sessionEnv.section.mcp')} count={env.mcp.servers.length}>
        ...
        {env.mcp.servers.map((s) => (
          <Row key={s.name} testId={`session-env-mcp-${s.name}`} state={s.state}>
            <span className="font-mono text-compact text-app-text">{s.name}</span>
      ...
      <Section icon={<Webhook size={11} />} title={t('sessionEnv.section.hooks')} count={env.hooks.length}>
        ...
        {hookEvents.map((event) => (
          <div key={event} data-testid={`session-env-hook-event-${event}`}>
            <div className="pb-0.5 pt-1 font-mono text-mini text-app-text-tertiary">{event}</div>
      ...
      <Section icon={<Terminal size={11} />} title={t('sessionEnv.section.commands')} count={env.commands.length}>
        ...
        {env.commands.map((c) => (
          <Row key={`${c.kind}-${c.name}`} testId={`session-env-command-${c.name}`}>
            <span className="font-mono text-compact text-app-text" title={c.file}>/{c.name}</span>
      ...
      <Section icon={<ShieldCheck size={11} />} title={t('sessionEnv.section.permissions')} count={env.permissions.rules.length}>
        ...
        {env.permissions.rules.map((r, i) => (
          <Row key={`${r.effect}-${r.rule}-${i}`} testId={`session-env-rule-${i}`}>
            ...
            <span className="font-mono text-mini text-app-text">{r.rule}</span>
      ...
      <Section icon={<FileCog size={11} />} title={t('sessionEnv.section.files')} count={env.settingsFiles.length}>
        {env.settingsFiles.map((f) => (
          <Row key={f.path} testId={`session-env-file-${f.source}`} state={f.exists ? undefined : 'excluded'}>
            ...
            <span className="break-all font-mono text-mini text-app-text-muted">{f.path}</span>
        ...
        {env.mcp.source && (
          <Row testId="session-env-file-mcp">
            <Chip mono>mcp</Chip>
            <span className="break-all font-mono text-mini text-app-text-muted">{env.mcp.source}</span>
          </Row>
        )}
      </Section>

      <p className="text-mini text-app-text-muted">{t('sessionEnv.readonly')}</p>
    </div>
  );
}
...
function Detail({ children, muted }: { children: React.ReactNode; muted?: boolean }) {
  return (
    <span className={`w-full break-all font-mono text-mini ${muted ? 'text-app-text-muted' : 'text-app-text-secondary'}`}>
      {children}
    </span>
  );
}
...
function Note({ children, testId }: { children: React.ReactNode; testId?: string }) {
  return <div data-testid={testId} className="break-words text-mini text-app-text-muted">{children}</div>;
}
