    ...
    >
      {posizione && (
        <div
          ...
          className="absolute left-1/2 top-4 -translate-x-1/2 rounded-full bg-black/60 px-3 py-1 text-compact leading-4 text-white"
        >{posizione}</div>
      )}
      <button
        onClick={onClose}
...
export function PreviewMedia({ path, paths, variant, onOpenTab }: {
  ...
  const media = !isPreviewablePath(corrente) ? (
    <a
      ...
      className={`flex items-center gap-2 px-2.5 py-2 text-left text-compact leading-4 ${
        variant === 'card'
