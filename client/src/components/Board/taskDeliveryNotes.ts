...
export function deliveryNotesToFold(comments: readonly TaskComment[]): Set<string> {
  ...
  for (const comment of comments) {
    ...
    if (comment.kind === 'comment' && comment.content.trim()) replies.add(key);
    ...
    if (comment.kind === 'delivery' && !comment.media?.length && !/[?]|\]\s*[([]/.test(comment.content) && replies.has(key)) notes.add(comment.id);
  }
  return notes;
}
