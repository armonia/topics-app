...
export function DispatchChip({ state, error, deliveredBy, hasWork = true }: { state: string; error?: string | null; deliveredBy?: string | null; hasWork?: boolean }) {
  ...
  return (
    <span
      ...
      data-state={state}
      className={`inline-flex shrink-0 items-center gap-1 rounded px-1.5 py-0.5 text-compact leading-4 md:text-mini ${chip.cls}`}
...
export function QueueReasonChip({ reason }: { reason: QueueReason }) {
  ...
  return (
    <span
      ...
      className={`inline-flex min-w-0 shrink items-center rounded px-1.5 py-0.5 text-compact leading-4 md:text-mini ${QUEUE_TONE_CLS[reason.tone]}`}
      title={title}
    >
...
export function LabelChip({ label, source }: { label: TaskLabel; source: LabelSource }) {
  ...
  return (
    <span
      ...
      className={`inline-flex shrink-0 items-center gap-1 rounded px-1.5 py-0.5 text-compact leading-4 md:text-mini ${cls}`}
    >
