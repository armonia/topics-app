    ...
    >
      ...
      {planPanel && (
        <div className="space-y-1">
          ...
          <textarea
            ...
            className="w-full text-prose leading-snug bg-surface border border-app-border rounded px-2 py-1.5 resize-y font-mono"
          />
      ...
      {(stepped ? [current] : questions).map((q) => {
        ...
        <fieldset key={`${toolCallId}-q-${qIdx}`} className="space-y-1.5">
          ...
          <legend className="text-prose leading-snug font-medium text-app-text">
          ...
          <div className="space-y-0.5">
            {(() => {
              ...
              return q.options.map((opt, oIdx) => {
                ...
                return (
              <label key={`${toolCallId}-q-${qIdx}-o-${oIdx}`} className="flex items-start gap-2 text-prose cursor-pointer hover:bg-app-hover rounded px-1.5 py-1">
                <input
                  type={inputType}
            ...
            <label className="flex items-start gap-2 text-prose cursor-pointer hover:bg-app-hover rounded px-1.5 py-1">
              <input
                type={inputType}
                name={q.multiSelect ? undefined : `${toolCallId}-q-${qIdx}`}
                value={OTHER}
              ...
              <div className="flex-1 min-w-0">
                ...
                <textarea
                  ...
                  className="mt-1 w-full text-prose bg-surface border border-app-border rounded px-2 py-1.5 resize-none"
                />
              </div>
            </label>
            )}
      ...
      <div className="flex justify-end items-center gap-2">
        {stepped && step > 0 && (
          <button
            ...
            className="px-3 py-1.5 text-prose font-medium rounded-md text-app-text-secondary hover:bg-app-hover disabled:opacity-40 transition-colors"
          >
            {tr('ask.back')}
          </button>
        )}
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-1.5 text-prose font-medium rounded-md bg-primary text-white hover:bg-primary-hover disabled:bg-app-text-muted/30 disabled:text-app-text-muted disabled:cursor-not-allowed transition-colors"
        >
...
function ElicitationForm({ requestedSchema, message, toolCallId, submitting, error, onSubmit }: ElicitationProp) {
  ...
  if (!fields) {
    return (
      ...
      >
        {message && <div className="text-prose leading-snug text-app-text">{message}</div>}
        <textarea
          value={jsonText}
          ...
          className="w-full text-prose font-mono bg-surface border border-app-border rounded px-2 py-1.5 resize-none"
        />
        {error && <div className="text-compact text-red-500">{error}</div>}
        <div className="flex justify-end">
          <button type="submit" disabled={submitting} className="flex items-center gap-1.5 px-3.5 py-1.5 text-prose font-medium rounded-md bg-primary text-white hover:bg-primary-hover disabled:bg-app-text-muted/30 disabled:cursor-not-allowed">
  ...
  return (
    ...
    >
      ...
      {fields.map((f) => (
        <div key={f.name} className="block text-prose">
          ...
          {f.type === 'boolean' ? (
            ...
            <input
              ...
              disabled={submitting}
              aria-label={f.name}
              className="mt-0.5 w-full text-prose bg-surface border border-app-border rounded px-2 py-1.5"
            />
          )}
        </div>
      ))}
      {error && <div className="text-compact text-red-500">{error}</div>}
      <div className="flex justify-end">
        <button type="submit" disabled={!allRequiredFilled || submitting} className="flex items-center gap-1.5 px-3.5 py-1.5 text-prose font-medium rounded-md bg-primary text-white hover:bg-primary-hover disabled:bg-app-text-muted/30 disabled:cursor-not-allowed">
          {submitting ? <Loader2 size={13} className="animate-spin" /> : <Send size={13} />}
          {submitting ? tr('ask.sending') : tr('ask.send')}
        </button>
      </div>
  ...
  return (
    ...
    >
      ...
      <textarea
        ...
        rows={3}
        placeholder={tr('ask.answerPlaceholder')}
        className="w-full text-prose bg-surface border border-app-border rounded px-2 py-1.5 resize-none"
      />
      {error && <div className="text-compact text-red-500">{error}</div>}
      <div className="flex justify-end">
        <button type="submit" disabled={submitting || !text.trim()} className="flex items-center gap-1.5 px-3.5 py-1.5 text-prose font-medium rounded-md bg-primary text-white hover:bg-primary-hover disabled:bg-app-text-muted/30 disabled:cursor-not-allowed">
          {submitting ? <Loader2 size={13} className="animate-spin" /> : <Send size={13} />}
          {submitting ? tr('ask.sending') : tr('ask.send')}
        </button>
      </div>
