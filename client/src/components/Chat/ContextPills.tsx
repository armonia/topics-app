...
export function ContextPills({ files, onRemove, onToggle, excludedPaths, compact = false }: ContextPillsProps) {
  ...
  return (
    <div data-testid="context-pills" className={`flex items-center gap-1.5 overflow-x-auto scrollbar-hide ${compact ? '' : 'px-3 py-1.5 border-t border-app-border'}`}>
      {!compact && (
        <span className="text-mini text-app-text-muted font-medium mr-0.5 flex-shrink-0">Context</span>
      )}
      {files.map(file => (
        <ContextPill
          key={file.path}
...
function ContextPill({ file, excluded, onToggle, onRemove }: {
  ...
  return (
    <span
      ...
      className={`context-pill inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md border text-mini font-medium flex-shrink-0 transition-all ${
    ...
    >
      ...
      {file.tokens != null && file.tokens > 0 && (
        <span className="text-mini opacity-60 flex-shrink-0">
