    ...
    <div className="flex flex-col h-full">
      ...
      {showFilters && (
        <div className="flex items-center gap-2 px-2.5 py-1.5 border-b border-app-border/40 flex-shrink-0">
          ...
          {hasActiveFilters && (
            <button onClick={() => { setStatusFilter(''); setAgentFilter(''); setSearchInput(''); }} className="text-[11px] text-app-text-muted hover:text-primary transition-colors">Clear</button>
          )}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        {filteredLive.length > 0 && (
          <>
            <div className="px-2.5 pt-2 pb-1">
              <span className="text-[11px] font-medium text-app-text-muted uppercase tracking-wider">Live</span>
            </div>
            {filteredLive.map(s => (
        ...
        {(historySessions.length > 0 || loading) && (
          <div className="px-2.5 pt-2 pb-1">
            <span className="text-[11px] font-medium text-app-text-muted uppercase tracking-wider">History</span>
          </div>
        )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-3 py-1.5 border-t border-app-border/40 flex-shrink-0">
          ...
          <span className="text-[11px] text-app-text-muted tabular-nums">{page + 1} / {totalPages}</span>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          <span className={`text-[11px] px-1 py-px rounded font-medium flex-shrink-0 ${statusBadge(session.status)}`}>
            {statusLabel(session.status)}
          </span>
          {session.isLive && (
            <span className="text-[11px] px-1 py-px rounded bg-green-500/10 text-green-500 font-medium flex-shrink-0">LIVE</span>
          )}
        </div>
        <div className="flex items-center gap-2 mt-0.5">
          {session.topicName && (
            <span className="text-[11px] text-app-text-secondary truncate max-w-[140px]" title={session.topicName}>
              {session.topicName}
            </span>
          )}
          <span className="text-[11px] text-app-text-muted">{formatDate(session.startedAt)}</span>
          <span className="text-[11px] text-app-text-muted">{formatDuration(session.startedAt, session.completedAt)}</span>
          {session.totalTokens > 0 && (
            <span className="text-[11px] text-app-text-muted">{formatTokens(session.totalTokens)} tok</span>
          )}
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-2.5 py-2 border-b border-app-border/40 flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[11px] px-1 py-px rounded font-medium ${statusBadge(session.status)}`}>
              {statusLabel(session.status)}
            </span>
          </div>
        ...
        {onOpenInPane && (
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 rounded text-[11px] font-medium text-purple-500 hover:bg-purple-500/10 transition-colors flex-shrink-0"
            title="Open in pane"
          >
      ...
      {session.errorMessage && (
        <div className="mx-2.5 mt-2 px-2 py-1.5 rounded bg-red-500/10 text-[11px] text-red-400 flex-shrink-0">
          {session.errorMessage}
        </div>
      )}
  ...
  return (
    <div className={`flex gap-1.5 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`max-w-[85%] rounded-lg px-2.5 py-1.5 ${
        ...
        {isTruncated && (
          <button onClick={() => setShowFull(true)} className="text-[11px] text-primary hover:underline mt-0.5">
            Show more
          </button>
        )}
        {message.thinking && (
          <details className="mt-1">
            <summary className="text-[11px] text-app-text-muted cursor-pointer hover:text-app-text-secondary">Thinking</summary>
            <p className="text-[11px] text-app-text-muted mt-0.5 whitespace-pre-wrap">
        ...
        {message.timestamp && (
          <div className="text-[11px] text-app-text-muted mt-0.5 text-right">
