...
// A CYCLE, and it holds: `tabLink` imports `openTaskInApp` from here, and this
// module imports `openTabInApp` from there. Neither side touches the other at
// module-evaluation time — both are function declarations called long after the
// graph is wired — so whichever module loads first gets a live binding that is
// already filled by the time anybody calls it. The alternative (a dynamic
// import) would turn every notification click into an async hop for no gain.
