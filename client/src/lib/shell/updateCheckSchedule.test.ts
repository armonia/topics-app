...
describe('when the app looks for a new version', () => {
  ...
  test('a dev install is NOT exempt: the checks run there too', async () => {
    let calls = 0;
    // For one day this returned a no-op on the machine that builds the app, to
    // stop a banner that named the wrong number. Measured the day after: shell
    // 2.2.264, published 2.2.277, thirteen releases of silence - and a
    // requirement (STATUSLINE-03c) that says the shell's update must not be
    // hushed in development. The confusing sentence is cured where sentences
    // are made (`shellUpdateNotice`); the check goes back to running.
    const stop = startUpdateChecks(() => { calls++; }, { bootDelayMs: 5, periodMs: 10 });
    try {
      await wait(40);
      expect(calls).toBeGreaterThan(1);
