...
const EN = {
  ...
  'board.dispatch.resourcesHint': 'A new agent starts only while the whole machine’s load and memory sit under the two thresholds. The fixed number does not apply here.',
  'board.dispatch.loadThreshold': 'Load threshold (load per core)',
  'board.dispatch.memThreshold': 'Memory threshold (used over total)',
  'board.dispatch.band.green': 'Recommended band: it brakes before the person at the keyboard feels it.',
  'board.dispatch.band.amberLow': 'Cautious: the queue moves slowly and often waits even on a quiet machine.',
  'board.dispatch.band.amberHigh': 'Permissive: the brake bites when the machine is already heavy.',
  'board.dispatch.band.redLow': 'Too low: the queue almost never starts, only the first agent gets through.',
  'board.dispatch.band.redHigh': 'Too high: the machine turns unusable before the brake steps in.',
  'board.dispatch.liveLoad': 'Now: load {load} on {cores} cores, {ratio} per core',
  'board.dispatch.liveMem': 'Now: {used} of {total} GB used ({pct}%)',
  'board.dispatch.liveMemUnknown': 'Now: memory not measured on this system, it does not brake.',
  'board.dispatch.liveLoading': 'Reading the machine…',
  'board.dispatch.verdictGo': 'Right now a new agent would start.',
  'board.dispatch.verdictWait': 'Right now a new agent would wait: {axis} over the threshold.',
  'board.dispatch.verdictFirst': 'Over the threshold, but with no agent working the first one starts anyway.',
  'board.dispatch.axisLoad': 'load',
  'board.dispatch.axisMem': 'memory',
