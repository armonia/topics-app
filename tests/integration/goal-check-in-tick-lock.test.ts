...
async function run(periodS: number, tickS: number, judgeS: number, horizonMin: number, offS = 0) {
  ...
  let judged = 0; const sent: string[] = []; const logs: string[] = [];
  const onTurnEnd = createGoalContinuation({
    db: ctx.db,
    judge: () => { judged++; return new Promise<string>((r) => at(clock + judgeS * S, () => r("continue"))); },
  ...
  return { judged, sent: sent.length, cont: g?.continuations, lastLogs: logs.slice(-4) };
}
