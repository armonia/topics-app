 ...
 *   2. a POSIX id (the lying tail: it looks like an id, it has no backslash)
...
const POSIX_PATH = "/Users/someone/Projects/topics-app";
...
describe("migration: an id written with the whole Windows path goes back to the folder", () => {
  ...
  test("the guard against a green that did nothing: exactly the broken rows move", () => {
    ...
    db.run(`INSERT INTO tasks (id, project_id, text) VALUES ('t3', ?, 'c')`, [projectIdForPath(POSIX_PATH)]);
    ...
    const still = db.query(`SELECT count(*) AS n FROM tasks WHERE project_id = ?`).get(projectIdForPath(POSIX_PATH)) as { n: number };
    expect(still.n).toBe(1);
    const left = db.query(`SELECT count(*) AS n FROM tasks WHERE instr(project_id, char(92)) > 0`).get() as { n: number };
    expect(left.n).toBe(0);
  });

  test("the lying tail: a POSIX id resembles the criterion without being it, and is left alone", () => {
    const db = dbBefore();
    const posix = projectIdForPath(POSIX_PATH);
    db.run(`INSERT INTO tasks (id, project_id, text) VALUES ('t1', ?, 'a')`, [posix]);
    db.run(`INSERT INTO board_settings (project_id, max_agents) VALUES (?, 7)`, [posix]);
    db.run(`INSERT INTO board_memory (id, project_id, content) VALUES ('m1', ?, 'n')`, [posix]);

    db.run(MIGRATION_SQL);

    expect((db.query("SELECT project_id AS p FROM tasks").get() as { p: string }).p).toBe(posix);
    expect((db.query("SELECT max_agents AS m FROM board_settings WHERE project_id = ?").get(posix) as { m: number }).m).toBe(7);
    expect((db.query("SELECT project_id AS p FROM board_memory").get() as { p: string }).p).toBe(posix);
  });
