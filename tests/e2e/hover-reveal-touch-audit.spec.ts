...
const OTHER_BRANCH = "tocco/da-cancellare";
...
async function openBranches(page: import("@playwright/test").Page) {
  ...
  const riga = page.locator(`[data-testid="branch-row"][data-branch="${OTHER_BRANCH}"]`);
...
test.describe("comandi nascosti dietro l'hover, col dito", () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    execFileSync("git", ["branch", OTHER_BRANCH], { cwd: tmpDir });
  });
  ...
  test("HOVER-TOUCH-01: «elimina branch» è raggiungibile tenendo premuto", async ({
    ...
    await longPress(page, `[data-testid="branch-row"][data-branch="${OTHER_BRANCH}"]`, { until: voce });
    await expect(voce).toBeVisible({ timeout: 5000 });
    await voce.click();

    // E porta dove deve: il dialogo di conferma, col nome del branch.
    await expect(page.getByText(OTHER_BRANCH, { exact: false }).last()).toBeVisible({ timeout: 5000 });
  ...
  test("HOVER-TOUCH-02: nessun bersaglio invisibile resta cliccabile", async ({
    ...
    const bottone = page.locator(
      `[data-testid="branch-row"][data-branch="${OTHER_BRANCH}"] [data-testid="branch-delete"]`,
    );
