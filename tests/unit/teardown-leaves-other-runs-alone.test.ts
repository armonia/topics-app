...
describe("the teardown asks before killing", () => {
  test("the source consults the lock holder before the kill on the port", () => {
    ...
    // `killTestListeners` and no longer a hand-written `lsof`: who holds this
    // port and whether it is a test server live in tests/e2e/helpers/
    // (platform.ts, port-guard.ts). What this test guards is unchanged - the
    // ORDER between asking the lock and killing.
    const kill = src.indexOf("killTestListeners(TEST_PORT");
