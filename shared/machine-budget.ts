 ...
 *
 * THE BUDGET IS A CEILING, NOT A RIGHT, AND THE SHARE IS ON WHAT IS FREE. What is
 ...
 *
 * Decided by the owner of the machine on 2026-09-14: the percentage is a
 * percentage OF WHAT IS FREE, so whatever Topics did not start has priority.
 * The rule used to be `min(share x cores, free)`: at 80% on 12 cores with 7.3
 * cores taken by others, Topics could take ALL of the 4.7 that were left. Now
 * it takes 3.8 and leaves 0.9 to whoever grows. The opposite policy (take the budget
...
export interface MachineBudget {
  ...
  cpuCoreUnits: number;
  /** `min(cpuCoreUnits, share x (cores - other))`: our share OF WHAT IS FREE,
   *  the ceiling reality grants right now while letting the rest grow. */
...
export function machineBudget(sample: MachineBudgetSample, share: number): MachineBudget {
  ...
  //
  // THE SHARE IS ON WHAT IS FREE: `share x free`, not `free`. Whatever is not
  // ours always keeps its slice of what is left over, so a process that grows
