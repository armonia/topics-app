...
export const PREVIEW_RULE = [
  ...
  "· VIDEO .webm/.mp4/.mov ≤20s — proving the delivery takes TWO OR MORE STATES (appears, stays, disappears; scroll, open/close, streaming, a multi-step flow): a still screenshot cannot prove a behaviour. Never record it with Playwright or any browser you launch on this machine (no Chromium here): record the task's browser tab (open_browser_pane) with `screencapture -V <seconds> -R <x,y,w,h> <file>.mov`. On the Windows PC or in CI a Playwright clip is fine.",
...
export const CODE_GATES_RULE = [
  ...
  "E2E NEVER RUNS ON THIS MACHINE (decided 2026-09-15): do not run `bun run check:e2e-touched` (only `--list`, which launches nothing), `playwright test` in any form, or a client build meant for e2e, and never install or launch a browser, in the foreground or in the background. In 15 hours on 2026-09-15 agents ran 21 `check:e2e-touched`, 71 raw `playwright test` and 27 client builds by themselves, 99 of those 119 backgrounded with `&`, while the board re-ran the e2e at delivery anyway. Write or change the e2e spec under `tests/e2e/` and commit it: the proof comes from the CI of your branch, which the board reads when you deliver. Targeted `bun test <file>` stays allowed.",
].join("\n");
...
export const QUEUE_REASON_KINDS: QueueReasonKind[] = [
  ...
  'resource_floor', 'resource_pressure', 'spend_cap', 'node_unreachable', 'plan_window',
