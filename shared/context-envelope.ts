/**
 * Il Context Envelope canonico — UNA dichiarazione per i due lati del filo.
 *
 * È la risposta di `GET /api/topics/:id/context-preview` e di
 * `…/context-snapshots`: quello che l'ispettore contesto renderizza. Fino al
 * 29/07 il client ne teneva una RISCRITTURA a mano in `lib/api.ts`, con i
 * tipi rinominati `Envelope*` e un `diagnostics` espanso inline — sette tipi
 * ricopiati, ognuno col suo "Mirrors server/context/envelope.ts:…".
 *
 * La logica di assemblaggio (`assembleTopicContext`, `adaptEnvelope`) resta in
 * `server/context/envelope.ts`: qui c'è solo la FORMA, che è la sola cosa che
 * attraversa il filo.
 */
import type { ProviderChatMessage, ProviderContextStrategy } from './types';
...
// `ProviderContextStrategy` e `ProviderChatMessage` stanno in `shared/types.ts`:
// li importa anche `server/providers/types.ts` per dichiarare `contextStrategy`
// sull'interfaccia `AIProvider` senza dipendere da questo modulo.
...
// ────────────────────────────────────────────────────────────────────────────
// Payload per il provider (output di adaptEnvelope)
// ────────────────────────────────────────────────────────────────────────────

/**
 * Gli argomenti concreti passati a `provider.sendChat`, prodotti da
 * `adaptEnvelope(envelope)` secondo `envelope.providerStrategy`.
 *
 * `userContent` è ciò che l'handler passa come argomento `message`;
 * `history` (quando c'è) finisce in `options.history`; `options.tools` lo
 * mette il chiamante (registrazione dei browser tool), non l'adapter — è
 * agnostico rispetto al provider.
 *
 * `adaptationNotes[]` è puramente diagnostico: l'ispettore le renderizza così
 * l'utente capisce come è stato rimodellato il suo envelope (es. "7 blocchi di
 * sistema inlineati nel turno utente").
 *
 * Generico su `TTool` perché il tipo del tool è quello dell'SDK Anthropic, che
 * sta solo fra le dipendenze del server: il server istanzia
 * `ProviderPayload<Tool>`, il client legge il default `unknown` — che è
 * esattamente quanto ne sa (non li renderizza).
 */
export interface ProviderPayload<TTool = unknown> {
  userContent: string;
  history?: ProviderChatMessage[];
  options?: {
    model?: string;
