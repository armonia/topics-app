...
# Creare l'Apple Account, accettare il contratto che impegna [cliente], e
...
csr() {
  local email nome
  read -r -p "Email dell'Apple Account (quello NUOVO, titolare di [cliente]): " email
...
segreti() {
  ...
  if [ -n "$identita" ]; then
    ...
    read -r -p 'APPLE_SIGNING_IDENTITY (es. "Developer ID Application: [cliente] (TEAMID)"): ' identita
    printf '%s' "$identita" | gh secret set APPLE_SIGNING_IDENTITY
  fi
...
[cliente] (D-U-N-S [cliente]), pagare.
USO
    ;;
esac
